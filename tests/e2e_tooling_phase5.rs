...
fn test_lsp_initialize() {
    ...
    init_req.insert("method".to_string(), JsonValue::String("initialize".to_string()));
    init_req.insert("params".to_string(), JsonValue::Object(BTreeMap::new()));

    let resp = server.handle_message(&JsonValue::Object(init_req)).expect("Expected response");
    ...
    // Full sync = 1
    assert_eq!(capabilities.get("textDocumentSync").and_then(|s| s.as_i64()), Some(1));
    assert_eq!(capabilities.get("hoverProvider").and_then(|h| h.as_bool()), Some(true));
    assert_eq!(capabilities.get("definitionProvider").and_then(|d| d.as_bool()), Some(true));

    let comp = capabilities.get("completionProvider").expect("Expected completionProvider");
    assert_eq!(comp.get("resolveProvider").and_then(|r| r.as_bool()), Some(false));
}
...
fn test_lsp_diagnostics_did_open_and_did_change() {
    ...
    doc_info.insert("text".to_string(), JsonValue::String(invalid_source.to_string()));
    ...
    let notif = server.handle_message(&JsonValue::Object(did_open)).expect("Expected notification");
    assert_eq!(notif.get("method").and_then(|m| m.as_str()), Some("textDocument/publishDiagnostics"));

    let diags = notif.get("params")
    ...
    did_change_params.insert("textDocument".to_string(), JsonValue::Object(doc_change_info));

    let mut change_item = BTreeMap::new();
    change_item.insert("text".to_string(), JsonValue::String(valid_source.to_string()));
    did_change_params.insert("contentChanges".to_string(), JsonValue::Array(vec![JsonValue::Object(change_item)]));

    let mut did_change = BTreeMap::new();
    did_change.insert("jsonrpc".to_string(), JsonValue::String("2.0".to_string()));
    did_change.insert("method".to_string(), JsonValue::String("textDocument/didChange".to_string()));
    did_change.insert("params".to_string(), JsonValue::Object(did_change_params));

    let fixed_notif = server.handle_message(&JsonValue::Object(did_change)).expect("Expected notification");
    let fixed_diags = fixed_notif.get("params")
        .and_then(|p| p.get("diagnostics"))
        .and_then(|d| d.as_array())
        .expect("Expected diagnostics array");
...
"#;
    ...
    comp_req.insert("method".to_string(), JsonValue::String("textDocument/completion".to_string()));
    comp_req.insert("params".to_string(), JsonValue::Object(comp_params));

    let comp_resp = server.handle_message(&JsonValue::Object(comp_req)).expect("Expected response");
    let items = comp_resp.get("result").and_then(|r| r.as_array()).expect("Expected items array");

    let labels: Vec<&str> = items.iter().filter_map(|it| it.get("label").and_then(|l| l.as_str())).collect();
    assert!(labels.contains(&"function"), "Must contain 'function' keyword");
    ...
    assert!(labels.contains(&"Shape"), "Must contain AST interface Shape");
    assert!(labels.contains(&"calculate_area"), "Must contain AST function calculate_area");
    ...
    hover_req.insert("method".to_string(), JsonValue::String("textDocument/hover".to_string()));
    hover_req.insert("params".to_string(), JsonValue::Object(hover_params));

    let hover_resp = server.handle_message(&JsonValue::Object(hover_req)).expect("Expected response");
    let contents = hover_resp.get("result")
        .and_then(|r| r.get("contents"))
        .and_then(|c| c.get("value"))
        .and_then(|v| v.as_str())
        .expect("Expected hover markdown");
    assert!(contents.contains("calculate_area"), "Hover tooltip should contain function signature");
    ...
    def_req.insert("method".to_string(), JsonValue::String("textDocument/definition".to_string()));
    def_req.insert("params".to_string(), JsonValue::Object(def_params));

    let def_resp = server.handle_message(&JsonValue::Object(def_req)).expect("Expected response");
    let target_uri = def_resp.get("result")
    ...
    shutdown_req.insert("method".to_string(), JsonValue::String("shutdown".to_string()));

    let shut_resp = server.handle_message(&JsonValue::Object(shutdown_req)).expect("Expected response");
...
"#;
    ...
    assert!(fn_item.signature.contains("function multiply[T](a: int, b: int = 1) -> int"));
...
fn test_doc_run_directory_e2e() {
    ...
    fs::write(&file1, "## Core system functions\npub function init() -> bool\n return true\nend\n").unwrap();
    fs::write(&file2, "## Extra helper functions\npub function helper() -> void\nend\n").unwrap();

    // Run doc generator over directory
    let res = run_doc(&src_dir.to_string_lossy(), Some(&out_dir.to_string_lossy()), true, true);
...
fn test_lockfile_serialization_with_checksum() {
    let pkg = LockedPackage {
        ...
        source: format_git_source("https://github.com/alya-lang/http.git", None, Some("v1.2.0"), None, Some("a1b2c3d4e5f60718293041526374859607182930")),
        entry: "src/lib.alya".to_string(),
        checksum: "sha256:ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad".to_string(),
    ...
    assert!(serialized.contains("checksum = \"sha256:ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad\""));
