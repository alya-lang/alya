...
pub fn resolve_generics(program: &mut Program) {
    ...
    for stmt in &program.statements {
        if let Stmt::Function { name, type_params, .. } = stmt.inner_stmt() {
    ...
    while pass < 5 {
        ...
        for mut spec_fn in current_specs {
            monomorphize_stmt(&mut spec_fn, &generic_funcs, &mut known_vars, &mut specializations);
        }
    ...
    match stmt {
        Stmt::Let { name, type_ann, value } => {
            ...
            if let Some(ann) = type_ann {
                ...
                if let Some(Stmt::Function { return_type: Some(rt), .. }) = specializations.get(callee) {
...
fn monomorphize_expr(
    ...
    match expr {
        Expr::Call { name, args } => {
            ...
            let (target_template_name, explicit_type_opt) = if let Some(_template) = generic_funcs.get(name) {
                    ...
                    *name = specialized_name.clone();
                    ...
                    if !specializations.contains_key(&specialized_name) {
                        let spec_param_types = t_param_types
                            ...
                            .collect();
                        let spec_return_type = t_return_type.as_ref().map(|t| substitute_type_str(t, &subst));
...
fn substitute_in_stmts(stmts: &[Stmt], subst: &HashMap<String, String>) -> Vec<Stmt> {
    stmts
        .iter()
        .map(|s| match s {
            Stmt::Let { name, type_ann, value } => Stmt::Let {
                name: name.clone(),
