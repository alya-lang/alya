...
pub fn emit_header(out: &mut String, os: OperatingSystem) {
    if matches!(os, OperatingSystem::MacOS) {
        ...
        emit_adrp_add(out, "x2", "alya_argv", os);
        out.push_str("    str x1, [x2]\n");
        // Windows Sockets require process-wide startup before any socket call.
        out.push_str("    sub sp, sp, #48\n");
        out.push_str("    movz x0, #0x202\n"); // MAKEWORD(2, 2)
        out.push_str("    adrp x1, alya_wsa_data\n");
        out.push_str("    add x1, x1, :lo12:alya_wsa_data\n");
        out.push_str("    bl WSAStartup\n");
